...
// CORRIGÉ :
//   • ProgressService().currentStudent → ProgressService().student
//   • Import storytelling_screen.dart et world_map_screen.dart présents
//   • Fond tropical dessiné en CustomPaint (aucune image externe)
//   • 5 mascottes sélectionnables
//   • Compatibilité avec progress_service.dart existant
// ═══════════════════════════════════════════════════════════
...
class _OnboardingScreenState extends State<OnboardingScreen>
  ...
  Future<void> _start() async {
    ...
    // CORRIGÉ : ProgressService().student (pas .currentStudent)
    // Premier lancement si aucun student préexistant → Storytelling
    final isFirst = ProgressService().student == null;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        // Fond tropical dessiné en CustomPaint
          ...
          builder: (_, __) => Stack(
            children: _starDecos.map((s) {
              final opacity = 0.4 + 0.6 * (0.5 + 0.5 *
                math.sin(_starsCtrl.value * math.pi * 2 + s.phase));
              return Positioned(
                left: s.x * size.width,
                top:  s.y * size.height,
                child: Opacity(
          ...
          child: Stack(
            ...
            children: [
              ...
              Container(
                ...
                child: Center(
                  child: Text(mascot.emoji, style: const TextStyle(fontSize: 68))),
              ),
              ..._buildMascotStars(mascot.color),
            ],
          ),
        ),
      ),
  ...
  List<Widget> _buildMascotStars(Color c) => [
    Positioned(top: 10, right: 32, child: _SpinStar(color: c, size: 22, delay: 0)),
    Positioned(top: 18, left: 30,  child: _SpinStar(color: const Color(0xFFFFD700), size: 18, delay: 300)),
    Positioned(bottom: 20, right: 28, child: _SpinStar(color: const Color(0xFFFFD700), size: 16, delay: 600)),
  ];
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: Center(
            child: Text(_mascottes[_selectedMascot].emoji,
...
class _TropicalPainter extends CustomPainter {
  ...
  void _drawSun(Canvas canvas, double cx, double cy) {
    canvas.drawCircle(Offset(cx, cy), 36, Paint()..color = const Color(0xFFFFE082).withOpacity(0.35));
    ...
    for (int i = 0; i < 8; i++) {
      ...
      canvas.drawLine(
        ...
        Offset(cx + 44*math.cos(a), cy + 44*math.sin(a)),
        rp,
      );
    }
  }
  ...
  void _drawCloud(Canvas canvas, double cx, double cy, double scale) {
    ...
    canvas.drawOval(Rect.fromCenter(center: Offset(cx, cy), width: 90*scale, height: 32*scale), p);
