...
class _IslandLevelsScreenState extends State<IslandLevelsScreen>
  ...
  List<Color> _getPalette() {
    switch (widget.niveauLabel) {
      case 'CP':  return [const Color(0xFF0D47A1), const Color(0xFF1976D2), const Color(0xFF42A5F5)];
      case 'CE1': return [const Color(0xFF1B5E20), const Color(0xFF2E7D32), const Color(0xFF66BB6A)];
      case 'CE2': return [const Color(0xFFBF360C), const Color(0xFFE64A19), const Color(0xFFFF7043)];
      case 'CM1': return [const Color(0xFF4A148C), const Color(0xFF7B1FA2), const Color(0xFFCE93D8)];
      case 'CM2': return [const Color(0xFF7F0000), const Color(0xFFD32F2F), const Color(0xFFEF9A9A)];
  ...
  String _getIslandEmoji() {
    switch (widget.niveauLabel) {
      case 'CP':  return '🌊'; case 'CE1': return '🌿';
      case 'CE2': return '🔥'; case 'CM1': return '🌙';
      case 'CM2': return '🚀'; default: return '⭐';
    }
  }

  Color get _main => _palette[1];
  ...
  Widget _buildDecos() => IgnorePointer(child: Stack(children: [
    Positioned(top: 100 + _floatAnim.value * 0.5, right: 28,
        child: const Text('⭐', style: TextStyle(fontSize: 20))),
    Positioned(top: 240 - _floatAnim.value * 0.7, left: 16,
        child: const Text('💫', style: TextStyle(fontSize: 16))),
    Positioned(top: 380 + _floatAnim.value * 0.4, right: 44,
  ...
  Widget _buildHeader(BuildContext ctx, int earned, int maxStar) => Padding(
    ...
    child: Row(children: [
      _glassBtn(onTap: () => Navigator.pop(ctx),
    ...
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: AnimatedBuilder(
          ...
          child: SizedBox(width: 140, child: Column(children: [
            // Étoiles au-dessus
            Row(mainAxisAlignment: MainAxisAlignment.center,
              children: List.generate(3, (s) => Padding(
                padding: const EdgeInsets.symmetric(horizontal: 2),
                child: Text(s < stars ? '⭐' : '☆',
                  style: TextStyle(fontSize: 15,
                    color: s < stars ? Colors.amber : Colors.white.withValues(alpha: 0.25))),
              ))),
            const SizedBox(height: 5),
            _NodeBody(index: i, level: level, stars: stars,
                unlocked: unlocked, active: active, done: done,
                palette: _palette),
          ])),
        ),
      ),
...
class _NodeBody extends StatelessWidget {
  ...
  const _NodeBody({required this.index, required this.level, required this.stars,
      required this.unlocked, required this.active, required this.done,
  ...
  Widget build(BuildContext context) {
    ...
    final glow = done ? Colors.green : active ? palette[1] : Colors.transparent;
    ...
    return Container(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(24),
          border: Border.all(
            color: active ? Colors.white.withValues(alpha: 0.88)
          ...
          Positioned(top: 0, left: 0, right: 0,
            child: Container(height: 22, decoration: BoxDecoration(
              gradient: LinearGradient(colors: [
                Colors.white.withValues(alpha: 0.28), Colors.transparent],
                begin: Alignment.topCenter, end: Alignment.bottomCenter),
...
class _PathPainter extends CustomPainter {
  ...
  const _PathPainter({required this.prevXFrac,
...
class _StarsBg extends CustomPainter {
  ...
  void paint(Canvas canvas, Size s) {
    ...
    for (int i = 0; i < 70; i++) {
      canvas.drawCircle(
        Offset(rng.nextDouble() * s.width, rng.nextDouble() * s.height * 0.55),
        rng.nextDouble() * 1.5 + 0.4,
        Paint()..color = Colors.white.withValues(alpha: 0.18 + rng.nextDouble() * 0.45));
    }
  }
