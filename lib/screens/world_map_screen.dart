...
import 'island_levels_screen.dart';        // ← nouveau fichier (ci-dessous)
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  student != null ? 'Bonjour, ${student.name} !' : 'LinguaKids',
                ...
                if ((student?.currentStreak ?? 0) > 0)
                  Row(children: [
                    ...
                    Text(
                      '${student!.currentStreak} jour(s) de suite !',
                      style: TextStyle(
  ...
  Widget _buildIslandList() {
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) => ListView.builder(
        ...
        itemBuilder: (ctx, i) => _BigIslandCard(
          island: islands[i],
...
// ─────────────────────────────────────────────────────────────
// Carte d'une grande île (CP, CE1, etc.)
// ─────────────────────────────────────────────────────────────
class _BigIslandCard extends StatelessWidget {
  final Island island;
  final double waveValue;
  final double delay;
  final VoidCallback onTap;

  const _BigIslandCard({
  ...
  static const _colors = {
    'CP':  [Color(0xFF1565C0), Color(0xFF1E88E5), Color(0xFF42A5F5)],
    'CE1': [Color(0xFF1B5E20), Color(0xFF2E7D32), Color(0xFF66BB6A)],
    'CE2': [Color(0xFFBF360C), Color(0xFFD84315), Color(0xFFFF7043)],
    'CM1': [Color(0xFF4A148C), Color(0xFF6A1B9A), Color(0xFFBA68C8)],
    'CM2': [Color(0xFF880E4F), Color(0xFFC62828), Color(0xFFEF5350)],
  };

  @override
  Widget build(BuildContext context) {
    final locked = !island.isUnlocked;
    final colors = _colors[island.label] ?? [Colors.grey, Colors.grey, Colors.grey];
    // Animation flottante unique par île
    final t = (waveValue + delay) % 1.0;
    final bob = locked ? 0.0 : _sin(t * 6.28) * 5.0;
                ...
                child: Column(
                  ...
                  if (!locked) ...[
                    ...
                    Row(children: [
                      // Étoiles mini
                      ...List.generate(
                        (island.totalLevels * 3).clamp(0, 18),
                        (i) => Icon(
                          i < island.starsEarned ? Icons.star : Icons.star_border,
                          size: 11,
...
class _WavePainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final p = Paint()..color = Colors.lightBlue..style = PaintingStyle.fill;
    final path = Path();
    path.moveTo(0, 60);
    for (double x = 0; x <= size.width; x += 10) {
      path.lineTo(x, 60 + 20 * _sin(x / 60));
    }
