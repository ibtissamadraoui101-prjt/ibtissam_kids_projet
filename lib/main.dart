...
void main() async {
  ...
  try { await ProgressService().init(); } catch (e) {
    debugPrint('⚠️ ProgressService: $e');
  }

  try { TtsService().init(); } catch (e) {
    debugPrint('⚠️ TTS: $e');
  }
  ...
  try { await SpeechService().init(); } catch (e) {
...
class _SplashScreenState extends State<_SplashScreen>
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 400), () {
      try { SoundService().startMusic('world_map'); } catch (_) {}
    });
    Future.delayed(const Duration(milliseconds: 700), () {
      try { TtsService().speak('Bonjour ! Bienvenue dans LinguaKids !'); } catch (_) {}
    });
  }
    ...
    return Scaffold(
      body: AnimatedBuilder(
        animation: Listenable.merge([
          _orbitAnim, _bounceAnim, _textAnim, _entryAnim, _dotsCtrl, _shootCtrl
        ]),
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                FadeTransition(
                  ...
                  child: Container(
                    ...
                    child: const Text('Salut ! Je suis Dino 🐉\nPrêt pour l\'aventure ?',
  ...
  List<Widget> _buildOrbitStars() {
    ...
    return List.generate(3, (i) {
      ...
      final rx = 95.0; final ry = 40.0; // ellipse orbit
      ...
      return Transform.translate(
        ...
        child: Text(emojis[i],
            style: TextStyle(fontSize: sizes[i],
                shadows: [Shadow(color: Colors.amber.withValues(alpha: 0.8),
                    blurRadius: 8)])),
      );
    });
  }
  ...
  Widget _buildClouds(Size size) => IgnorePointer(
    child: Stack(children: [
      Positioned(top: 60, left: -10,
        child: _cloud(160, 60, 0.08)),
      Positioned(top: 40, right: -5,
        child: _cloud(140, 55, 0.07)),
      Positioned(top: size.height * 0.7, left: 10,
        child: _cloud(120, 45, 0.06)),
    ]),
  );
...
class _DragonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawOval(Rect.fromCenter(
        center: Offset(cx, cy + 20), width: 80, height: 85),
        Paint()..color = const Color(0xFF9C27B0));

    // Ventre
    canvas.drawOval(Rect.fromCenter(
        center: Offset(cx, cy + 25), width: 46, height: 60),
    ...
    // Tête
    canvas.drawCircle(Offset(cx, cy - 30), 36,
    ...
    // Oreilles
    canvas.drawCircle(Offset(cx - 32, cy - 52), 10,
        Paint()..color = const Color(0xFFAB47BC));
    canvas.drawCircle(Offset(cx + 32, cy - 52), 10,
        Paint()..color = const Color(0xFFAB47BC));
    ...
    // Pupilles
    canvas.drawCircle(Offset(cx - 12, cy - 32), 7,
        Paint()..color = const Color(0xFF1A237E));
    canvas.drawCircle(Offset(cx + 14, cy - 32), 7,
        Paint()..color = const Color(0xFF1A237E));
    ...
    canvas.drawPath(tailPath, Paint()
      ...
      ..strokeWidth = 9
      ..strokeCap = StrokeCap.round);
    canvas.drawCircle(Offset(cx + 56, cy + 22), 6,
